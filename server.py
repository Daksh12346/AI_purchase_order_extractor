...
def sync_fetch_po_pdfs(req: GmailFetchRequest) -> dict:
  ...
  if not user_email or not app_password:
    raise HTTPException(
        status_code=400,
        detail="Gmail address and 16-digit App Password required.",
    )
  ...
  try:
    ...
    status, message_numbers = mail.uid(
        "search", None, f'X-GM-RAW "{full_raw_query}"'
    )

    if status != "OK" or not message_numbers or not message_numbers[0]:
      status, message_numbers = mail.uid(
          "search", None, 'X-GM-RAW "has:attachment filename:pdf"'
      )
    ...
    for uid in email_uids:
      ...
      for response_part in msg_data:
        ...
        for part in msg.walk():
          content_disposition = sanitize_header_str(
